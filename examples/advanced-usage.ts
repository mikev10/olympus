 ...
 *
 * This example demonstrates advanced features of Olympus:
...
import {
  createOlympusSession,
  getAgentDefinitions,
  getOlympusSystemPrompt,
  getDefaultMcpServers
} from '../src/index.js';

async function main() {
  console.log('=== Advanced Olympus Example ===\n');

  // Example 1: Custom agent configuration
  console.log('Example 1: Custom Agents');

  const customSession = createOlympusSession({
    config: {
      agents: {
        // Use a faster model for the orchestrator in dev
        olympus: { model: 'claude-sonnet-4-5-20250514' },
        // Disable some agents
  ...
  const customPrompt = getOlympusSystemPrompt({
...
`
  ...
  const fullCustomSession = createOlympusSession({
...
import { z } from 'zod';
import { createOlympusSession } from 'olympus-ai';
