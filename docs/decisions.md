...
- **Chosen:** replace the witness with self-attestation plus corroboration, and say so in those words rather than leaving the reader to assume the old arrangement. The runner writes a manifest beside every reply recording the exact argv and environment overrides; a recursive listing of both scratch directories, captured before the run; the clean-room assertion results; the exit code; start and end timestamps with the wall-clock duration; the CLI version; the model id the CLI reported, rather than one asserted for it; token usage; the bundle's SHA-256; and the integrity-echo comparison that says whether the reply carried the bundle's markers. The CLI's own session log is committed beside it with message bodies stripped and only metadata records kept. The run's outcome is derived from what happened and never declared — the same discipline the runtime uses for status, where the runtime works the status out and the model is never asked for it — and a run whose outcome is not `counted` is not a review, which `triage-review` refuses rather than triages.
- **Why this is worth having:** every field above is something a fabrication would have to get right, and get right consistently. Timestamps, token counts, a CLI version, a model name and an integrity echo have to agree with each other, across two vendors, on every unit, without divergence — and all of it is committed, so a later reader can hold one unit's manifests against the next's and against the session logs beside them. The bundle's hash is recorded independently in the prompt file, so the input a reviewer was given can be rebuilt and checked rather than taken on trust.
