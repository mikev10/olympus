...
- **A second integrity check, stronger than the nonce:** the vendor-reported input token count must reach a floor of the payload's bytes divided by 5. Measured ratios were 3.18 and 3.20 bytes per token, so an honest run clears it easily, while both measured failures (32,893 and 24,181 tokens) fall far beneath it. It is produced by the vendor's API service, or by the locally installed CLI process, rather than by the model whose text is under review, so the *model* cannot write it — which is the whole of the claim; a vendor that fabricates its own telemetry defeats this check and every other one here (D-TOOLING-02). It is a lower bound on what was taken in, not proof that every byte arrived, and it catches navigation as well as truncation. The nonce stays as an independent second check.
