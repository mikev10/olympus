...
This document explains the architectural differences between OpenCode's swappable master agent system and Claude Code's skill-based routing, and how Olympus bridges this gap elegantly.
...
class AgentOrchestrator {
  ...
  switchMaster(agentType: 'olympus' | 'oracle' | 'prometheus') {
...
orchestrator.switchMaster('olympus');    // Implementation mode
...
5. System: Switch master to Olympus  ← Context may be lost
6. Olympus: Implements (needs to re-read plan)
```
...
4. User: "Now implement it"
5. Claude: Transitions to olympus skill  ← Same context!
...
| `switchMaster('olympus')` | Invoke `olympus` skill |
...
4. **Fluid transitions** - `prometheus` → `olympus` happens naturally
