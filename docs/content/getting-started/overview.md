...
- **Structured Workflows** for complex features (AI-DLC: Inception → Construction pipeline)
...
**AI-DLC Workflow Commands**
- `/plan <description>` - Start structured AI-DLC workflow (Inception → Construction)
- `/continue` - Resume an interrupted workflow
- `/workflow-status` - View active workflow status
...
```bash
/ascent /ultrawork refactor API    # Activates: ultrawork + ascent guarantee
```
