...
**Options:**
- `--global` - Create global user config (`~/.claude/olympus/config.jsonc`)
...
**Example:**
```bash
# Initialize global config (~/.claude/olympus/config.jsonc)
olympus-ai init --global

# Create local project config (.claude/olympus.jsonc, overwrite if exists)
...
Permanently deletes all global learning data. This is irreversible.

**Options:**
- `--project` - Only forget project-specific learnings (keep global learnings). Requires `--confirm`.

**Example:**
```bash
# Forget global learnings
olympus-ai learn --forget

# Forget only project learnings (requires --confirm)
olympus-ai learn --forget --project --confirm
```
...
# Check file sizes
ls -lah ~/.claude/olympus/learning/
```
...
The configuration file (`~/.claude/olympus/config.jsonc` for global, `.claude/olympus.jsonc` for project) controls agent models, feature toggles, and integrations:
...
|----------|---------|---------|
| `ANTHROPIC_API_KEY` | Claude API key (checked by `config --validate`) | `sk-...` |
