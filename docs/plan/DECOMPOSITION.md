...
- `cacheReadTokens` is zero on the first task of a run and non-zero on a second task that shares its `stablePrefix`, and the two tasks differ only in `variableSuffix`. A driver that concatenated the two would read zero on both, so the split is what the number proves
