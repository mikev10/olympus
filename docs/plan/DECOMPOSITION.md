...
- the relay refuses a path outside its grant with 403, naming the path; discards any `x-api-key` and `authorization` the client sent, so a request carrying a key of the task's own is sent upstream with the provider's and never with both; ignores the client's `Host` and absolute-form targets, so no request reaches any origin but its upstream; and does not follow a redirect. Each is asserted against a local upstream the test owns, with no model call
...
- **the ledger.** Paid: `I4.model-credential-not-readable-by-the-task`, now a live external assertion in `@olympus-ai/driver-claude-code`. Added live: `I4.model-relay-forwards-only-its-grant`, external in `@olympus-ai/sandbox`, covering the refusals above. So `pending-baseline.json` lowers I4 from 1 to 0 and adds nothing pending. D-P5-20's reverse is carried out
