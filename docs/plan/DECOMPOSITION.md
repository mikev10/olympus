...
- the review seat is recorded: `GateResult` carries the author's and reviewer's `ModelIdentity` and whether independence is reduced, and an `EvidenceBundle` carries the `ModelIdentity` that produced it, so the author's family is read from the Vault and not from memory
- a write-once admission record in the Vault holds the `Run`, the resolved `Policy`, and the artifact paths each station locks, and `RunState` references it and carries what a resume needs: per-task attempt counts, recorded approvals, review seats, and whether the current station's work is complete. A resume reads its level, policy, and bounds from there; the caller cannot restate them
**Out of scope:**
...
- a review seat whose reviewer shares the author's `ModelIdentity.family` is refused at L3 and the run does not advance; at L0–L2 it is seated and the gate result and run state record reduced independence. A reviewer of a different family records full independence
...
- a run killed after any committed run state and resumed ends in the same station, task statuses, attempt counts, approvals, and review seats as the same run uninterrupted; a resume that asks for a different level or policy than the admission record holds is refused
- an over-request at any station the run will enter is refused at admission, naming the station, before anything is written
- **the ledger.** Paid: `I3.transition-reverifies-locks`, `I4.approval-outcome-gates-the-station`, `I6.review-seat-family-check`. Added pending: `I4.writable-globs-enforced-on-the-diff` (P6), `I5.unsafe-declaration-survives-composition` (P6). So `pending-baseline.json` lowers I3 from 1 to 0 and I6 from 1 to 0, leaves I4 at 2, and raises I5 from 4 to 5, each a deliberate edit in the diff
- `pnpm typecheck`, `pnpm lint`, `pnpm test`, and `pnpm conformance` all pass
- `git ls-files -- .plan/` prints nothing
