...
- the review seat is provisioned over the files its grants cover and nothing else — the locked spec, the acceptance tests, and the paths the diff changed, since the review contract grants no base tree (D-P6-08): a reviewer that opens the admitted task graph and an author-written file outside the diff is refused both
...
**Specified before any code, as P5's, P8's, and P6's were.** The five decisions this entry rests on are D-P12-01 to D-P12-05 in `docs/decisions.md`, each with the option not taken and how to reverse it.
**Deliver:** a **model relay** — a container the provider starts beside a sandbox that asks for one, holds the credential, and is destroyed with it. The CLI is pointed at the relay over the sandbox's internal network and sends a placeholder where a key would go; the relay discards every authentication header the client sent, writes the real credential into the one header the request names, and forwards to one fixed upstream origin over verified TLS, for the path prefixes the request grants and no others (D-P12-01, D-P12-03, D-P12-04). The container never holds the credential, so no process in it can read it: not the CLI, not a child of the CLI, not a later exec, not a file.
**Where it lives:** `packages/sandbox/src/local/relay.ts`, beside `proxy.ts` and built the way it is — source passed to `node --eval` in the same pinned image, `--read-only`, `--cap-drop ALL`, nothing mounted. The credential reaches the relay as `--env NAME` with the value set on the `docker` process, the technique A-P5-02 uses for the exec, so it is in no argument vector on the host. `LocalDockerProvider.create` takes the credentials it may hand to relays, by name; a spec names one and never carries a value, because a `SandboxSpec` is a record the runtime may keep and a secret in it is a secret in the record. `packages/drivers/claude-code` stops holding a credential: `ClaudeCodeDriverOptions.credential` is deleted, the exec carries the placeholder, and the driver exports the relay request its CLI needs (D-P12-05). The session check stays — a session that authenticated from any source but the variable the driver set is refused — and it now proves the CLI used the placeholder rather than something it found.
**The relay is independent of the egress mode** (D-P12-02). Under `allowlist` the sandbox's internal network carries the P10 proxy and the relay, and the relay's alias is added to `NO_PROXY` so the CLI does not send model traffic to a proxy that would refuse it. Under `deny-all` with a relay, the internal network carries the relay alone: the sandbox reaches one provider-owned endpoint and nothing else, by the same no-default-route construction P10 proved. `deny-all` without a relay is unchanged — `--network none`, loopback only, nothing started. The model's API host is not on the sandbox's allowlist in any of this unit's assertions; the relay is the only route to the model.
**Contract amendment, landed in this unit's pull request** as `A-P12-01` in `docs/decisions.md`:
- `SandboxSpec` gains an optional `relay: RelaySpec` — `upstream`, an `https` origin with no path; `paths`, a non-empty list of path prefixes; `header`, the one header the credential is written to; `credential`, the *name* of a credential the provider holds; and `urlVariable`, the environment variable the provider sets in the container to the relay's address, so the address stays the provider's to choose and no driver hard-codes it. An implementation MUST refuse a relay whose credential it was not given, whose upstream is not an `https` origin, or whose path list is empty, rather than provision a sandbox whose model calls fail later for a reason nothing recorded (I5). `StubSandboxProvider` cannot keep a credential out of a process it spawns on the host, so it refuses every spec that carries a relay, as it already declares it enforces no egress
**Out of scope:**
- wiring the relay into the line. `packages/api` keeps `StubDriver`, provisions build sandboxes with no relay, and never gives a verification sandbox one; the line obtaining the driver's relay request is I1's, with replacing the stub
- bounding what the task can spend through the relay. The CLI's `--max-budget-usd` bounds the CLI, and a task that calls the relay directly is bounded only by the sandbox's wall clock and the path grant. Before this unit the same task could copy the key and spend without limit after the sandbox was gone; after it, spend ends with the sandbox. Metering at the relay is R2's, stated as a known limit below
- reading or recording request and response bodies. The relay reads the request line and headers, which it must to check the path and replace the credential, and streams the body without parsing it. The P10 rule — the proxy reads nothing inside a connection — is untouched, because the relay is not in that connection: the CLI addresses the relay by name, and nothing the task sends through the proxy passes through it
- TLS between the sandbox and the relay. The internal network is the sandbox's own, carrying no default route and nothing but the provider's containers
- credentials minted short-lived by the vendor, OAuth or subscription sign-in, rotation, and any secret store. The provider takes a value it is given at construction
- a relay for any other vendor. `RelaySpec` names nothing Anthropic-specific, but only the Claude Code driver's is proven here; the Codex driver's is M2's
- a `Driver` contract method returning the relay request (D-P12-05). It arrives with the first consumer that holds a driver by its contract rather than its package, which is I1
**Conformance:** the P5 review's exploit, replayed, finds nothing — and finds the credential when it is passed on the exec as P5 passed it, so the assertion is shown able to see what it reports absent; the relay forwards its grant and refuses the rest; a sandbox's only route to the model is its own relay.
**Accept:**
- **the exploit, replayed.** With the real credential given to the provider and a real task run through the driver, a `SessionStart` hook — a child of the CLI — reads its own environment and every `/proc/*/environ` and `/proc/*/cmdline` it can open, a later exec given no options reads the same, and a search of every file the container user can read finds the credential value in none of them. `docker inspect` of the sandbox container finds it in no environment entry and no argument. The same assertion, run over a sandbox whose exec was given the credential as P5 gave it, finds it — the control that proves the scan is not blind (I8)
- the task completes: the CLI's session reports the driver's variable as its key source, the model answered, and all seven `driver.*` claim assertions pass with every model call routed through the relay. A relay that was not transparent to the CLI — streaming, cache headers, beta query strings — fails them
- the relay refuses a path outside its grant with 403, naming the path; discards any `x-api-key` and `authorization` the client sent, so a request carrying a key of the task's own is sent upstream with the provider's and never with both; replaces the client's `Host` and refuses absolute-form targets, so no request reaches any origin but its upstream (D-P12-08); and does not follow a redirect. Each is asserted against a local upstream the test owns, with no model call
- from inside a sandbox with a relay, the model's API host is unreachable directly, by name and by address, under `allowlist` and under `deny-all`: the relay is the only route, rather than the preferred one
- a spec naming a credential the provider was not given is refused at provisioning, naming the credential and never a value; so is an upstream that is not an `https` origin, and an empty path list. `StubSandboxProvider` refuses any spec with a relay
- `appliedControls()` records the relay — container, upstream, paths, header, credential name, and the exact `docker run` argv — and no recorded field contains the credential value
- the relay container is destroyed with its sandbox, and a leaked one fails the assertion, as the proxy's does
- a `deny-all` sandbox with no relay is byte-for-byte P10's: `--network none`, no container started, `appliedControls()` records `network: 'none'`
- the driver constructs without a credential and holds none: no field of it, no argument vector it builds, and no `ExecOptions` it passes contains the credential value
- **the ledger.** Paid: `I4.model-credential-not-readable-by-the-task`, now a live external assertion in `@olympus-ai/driver-claude-code`. Added live: `I4.model-relay-forwards-only-its-grant`, a runtime entry beside `I5.sandbox-egress-allowlist-enforced` (D-P12-06), covering the refusals above. So `pending-baseline.json` lowers I4 from 1 to 0 and adds nothing pending. D-P5-20's reverse is carried out
- `pnpm typecheck`, `pnpm lint`, `pnpm test` pass; then one minimal request confirms the Anthropic account is funded, and `pnpm --filter @olympus-ai/driver-claude-code test` runs once, because it calls a real model and any change under `packages/` makes its reconciled report stale; then `pnpm conformance` passes
- `git ls-files -- .plan/` prints nothing
