...
describe('loadCheckpoint Group 1B backward compatibility', () => {
  it('initializes all 5 validator fields with defaults when they are absent from checkpoint', async () => {
    const unit = makeUnit();
    const checkpoint = makeCheckpoint({ 'UNIT-001': unit });
    await saveCheckpoint(testDir, checkpoint);

    const loaded = await loadCheckpoint(testDir, workflowId);
    const loadedUnit = loaded?.construction_units?.['UNIT-001'];
  ...
  it('initializes fields for multiple units independently', async () => {
    const unit1 = makeUnit({ unitId: 'UNIT-001', quality_validation_status: 'completed' });
    const unit2 = makeUnit({ unitId: 'UNIT-002' });
    const checkpoint = makeCheckpoint({ 'UNIT-001': unit1, 'UNIT-002': unit2 });
    await saveCheckpoint(testDir, checkpoint);

    const loaded = await loadCheckpoint(testDir, workflowId);

    expect(loaded?.construction_units?.['UNIT-001'].quality_validation_status).toBe('completed');
    expect(loaded?.construction_units?.['UNIT-001'].traceability_status).toBe('not_started');

    expect(loaded?.construction_units?.['UNIT-002'].quality_validation_status).toBe('not_started');
    expect(loaded?.construction_units?.['UNIT-002'].coverage_percentage).toBeNull();
  });
  ...
  it('preserves coverage_percentage of 0 (falsy but valid)', async () => {
    ...
    const loaded = await loadCheckpoint(testDir, workflowId);
    expect(loaded?.construction_units?.['UNIT-001'].coverage_percentage).toBe(0);
  });
});
...
describe('loadCheckpoint Group 1D backward compatibility', () => {
  ...
  it('preserves existing Group 1D field values when they are already present', async () => {
    const unit = makeUnit({
      ...
      feature_doc_path: 'aidlc-docs/wf-compat-test/UNIT-001/feature-doc.md',
    ...
    expect(loadedUnit?.feature_doc_path).toBe('aidlc-docs/wf-compat-test/UNIT-001/feature-doc.md');
  ...
  it('initializes Group 1D fields for multiple units independently', async () => {
    const unit1 = makeUnit({ unitId: 'UNIT-001', security_scan_status: 'completed', adr_count: 4 });
    const unit2 = makeUnit({ unitId: 'UNIT-002' });
    const checkpoint = makeCheckpoint({ 'UNIT-001': unit1, 'UNIT-002': unit2 });
    await saveCheckpoint(testDir, checkpoint);
    ...
    expect(loaded?.construction_units?.['UNIT-001'].security_scan_status).toBe('completed');
    expect(loaded?.construction_units?.['UNIT-001'].adr_count).toBe(4);
    expect(loaded?.construction_units?.['UNIT-001'].feature_doc_status).toBe('not_started');

    expect(loaded?.construction_units?.['UNIT-002'].security_scan_status).toBe('not_started');
    expect(loaded?.construction_units?.['UNIT-002'].adr_count).toBe(0);
    expect(loaded?.construction_units?.['UNIT-002'].recreation_readiness_score).toBeNull();
  });
  ...
  it('preserves security_findings_critical of 0 (falsy but valid)', async () => {
    ...
    const loaded = await loadCheckpoint(testDir, workflowId);
    expect(loaded?.construction_units?.['UNIT-001'].security_findings_critical).toBe(0);
  });
  ...
  it('preserves adr_count of 0 (falsy but valid)', async () => {
    ...
    const loaded = await loadCheckpoint(testDir, workflowId);
    expect(loaded?.construction_units?.['UNIT-001'].adr_count).toBe(0);
  });
  ...
  it('preserves adr_entries when present', async () => {
    ...
    const loaded = await loadCheckpoint(testDir, workflowId);
    expect(loaded?.construction_units?.['UNIT-001'].adr_entries).toEqual([
  ...
  it('preserves impact_scan_report_path when present', async () => {
    const unit = makeUnit({
      impact_scan_report_path: 'aidlc-docs/wf/UNIT-001/docs/impact-scan.md',
    });
    const checkpoint = makeCheckpoint({ 'UNIT-001': unit });
    await saveCheckpoint(testDir, checkpoint);

    const loaded = await loadCheckpoint(testDir, workflowId);
    expect(loaded?.construction_units?.['UNIT-001'].impact_scan_report_path).toBe('aidlc-docs/wf/UNIT-001/docs/impact-scan.md');
  });
  ...
  it('preserves architecture_model_status when present', async () => {
    ...
    const loaded = await loadCheckpoint(testDir, workflowId);
    expect(loaded?.construction_units?.['UNIT-001'].architecture_model_status).toBe('updated');
  });
});
