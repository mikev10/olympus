 ...
 * Auto-Update System
 *
 * Provides version checking and auto-update functionality for Olympus.
 *
 * Features:
...
export const REPO_OWNER = 'mikev10';
export const REPO_NAME = 'olympus';
...
export const CONFIG_FILE = join(CLAUDE_CONFIG_DIR, '.olympus-config.json');

/**
 * Olympus configuration (stored in .olympus-config.json)
 */
...
/**
 * Read the Olympus configuration
 */
export function getOlympusConfig(): OlympusConfig {
  ...
  try {
    ...
    const config = JSON.parse(content) as OlympusConfig;
    return {
...
export function isSilentAutoUpdateEnabled(): boolean {
  return getOlympusConfig().silentAutoUpdate;
}
...
export function getInstalledVersion(): VersionMetadata | null {
  if (!existsSync(VERSION_FILE)) {
    ...
    try {
      ...
      const result = execSync('npm list -g olympus --json', {
        encoding: 'utf-8',
        timeout: 5000,
        stdio: 'pipe'
      });
      const data = JSON.parse(result);
      if (data.dependencies?.['olympus']?.version) {
        return {
          version: data.dependencies['olympus'].version,
...
export async function performUpdate(options?: {
  ...
  try {
    ...
    const tempScript = join(tempDir, `olympus-update-${Date.now()}.sh`);
    ...
    try {
      ...
      if (isWindows) {
        ...
        throw new Error(
          ...
          '  npm install -g olympus\n' +
          'Or visit: https://github.com/mikev10/olympus'
        );
      }
...
export function formatUpdateNotification(checkResult: UpdateCheckResult): string {
  if (!checkResult.updateAvailable) {
    return `Olympus is up to date (v${checkResult.currentVersion ?? 'unknown'})`;
  }
  ...
  const lines = [
    ...
    '',
    '  To update via npm (recommended):',
    '    npm update -g olympus-ai && olympus-ai install --force',
    '',
    '  Or use /update command for guided update',
    ''
  ];
...
const SILENT_UPDATE_STATE_FILE = join(CLAUDE_CONFIG_DIR, '.olympus-silent-update.json');
...
export async function silentAutoUpdate(config: SilentUpdateConfig = {}): Promise<UpdateResult | null> {
  const {
    ...
    autoApply = true,
    logFile = join(CLAUDE_CONFIG_DIR, '.olympus-update.log'),
    maxRetries = 3
  } = config;
