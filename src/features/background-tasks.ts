 ...
 *
 * Purpose: Manage asynchronous task execution with intelligent scheduling and concurrency control
 *
 * Integration:
 * - Bash tool: Use `run_in_background: true` for long-running commands
 * - Task tool: Use `run_in_background: true` for agent delegation
 * - TaskOutput tool: Retrieve results from background tasks
 *
 * Features:
 * - Decision heuristics: Auto-detect which commands should run in background
 * - Task lifecycle management: Track running tasks and completion
 * - Concurrency limits: Prevent resource exhaustion (default: 5 concurrent tasks)
 * - Pattern matching: Classify commands as long-running vs blocking
