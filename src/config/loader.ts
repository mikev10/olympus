...
export const DEFAULT_CONFIG: PluginConfig = {
  agents: {
    olympus: { model: 'claude-opus-4-5-20251101' },
    ...
    orchestratorOlympus: { model: 'claude-sonnet-4-5-20250929', enabled: true },
    olympusJunior: { model: 'claude-sonnet-4-5-20250929', enabled: true },
...
export function loadEnvConfig(): Partial<PluginConfig> {
  ...
  if (process.env.OLYMPUS_PARALLEL_EXECUTION !== undefined) {
    config.features = {
      ...config.features,
      parallelExecution: process.env.OLYMPUS_PARALLEL_EXECUTION === 'true'
    };
  }

  if (process.env.OLYMPUS_LSP_TOOLS !== undefined) {
    config.features = {
      ...config.features,
      lspTools: process.env.OLYMPUS_LSP_TOOLS === 'true'
    };
  }

  if (process.env.OLYMPUS_MAX_BACKGROUND_TASKS) {
    const maxTasks = parseInt(process.env.OLYMPUS_MAX_BACKGROUND_TASKS, 10);
    if (!isNaN(maxTasks)) {
      config.permissions = {
        ...config.permissions,
  ...
  if (process.env.OLYMPUS_ROUTING_ENABLED !== undefined) {
    config.routing = {
      ...config.routing,
      enabled: process.env.OLYMPUS_ROUTING_ENABLED === 'true'
    };
  }

  if (process.env.OLYMPUS_ROUTING_DEFAULT_TIER) {
    const tier = process.env.OLYMPUS_ROUTING_DEFAULT_TIER.toUpperCase();
  ...
  if (process.env.OLYMPUS_ESCALATION_ENABLED !== undefined) {
    config.routing = {
      ...config.routing,
