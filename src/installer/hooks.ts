...
/**
 * Olympus orchestration mode message
 * Injected when olympus/orchestrate/coordinate keywords detected
 */
export const OLYMPUS_MESSAGE = `<olympus-mode>
 ...
 * Enhanced stop hook that handles ultrawork, ascent, and todo continuation
 */
 ...
 * Can be disabled via OLYMPUS_USE_BUNDLED_HOOKS=0
 */
export function shouldUseBundledHooks(): boolean {
  // Explicitly disabled
  if (process.env.OLYMPUS_USE_BUNDLED_HOOKS === '0') {
    return false;
  }
  // Enabled by default for full feature set (including learning)
  return true;
}
