#!/bin/bash
# Olympus Uninstaller
# Completely removes all Olympus-installed files and configurations
...
echo -e "${BLUE}Olympus Uninstaller${NC}"
echo ""

# Claude Code config directory (always ~/.claude)
CLAUDE_CONFIG_DIR="$HOME/.claude"

echo "This will remove ALL Olympus components from:"
echo "  $CLAUDE_CONFIG_DIR"
echo ""
echo "Components to be removed:"
echo "  - Agents (oracle, librarian, explore, etc.)"
echo "  - Commands (olympus, ultrawork, plan, etc.)"
...
rm -f "$CLAUDE_CONFIG_DIR/agents/olympian.md"
rm -f "$CLAUDE_CONFIG_DIR/agents/prometheus.md"

# Remove commands
echo -e "${BLUE}Removing commands...${NC}"
rm -f "$CLAUDE_CONFIG_DIR/commands/olympus.md"
rm -f "$CLAUDE_CONFIG_DIR/commands/olympus-default.md"
...
rm -f "$CLAUDE_CONFIG_DIR/commands/ascent.md"
rm -f "$CLAUDE_CONFIG_DIR/commands/the-ascent.md"  # Legacy name
rm -f "$CLAUDE_CONFIG_DIR/commands/cancel-ascent.md"
...
rm -f "$CLAUDE_CONFIG_DIR/.olympus-version.json"
rm -f "$CLAUDE_CONFIG_DIR/.olympus-silent-update.json"
rm -f "$CLAUDE_CONFIG_DIR/.olympus-update.log"
rm -f "$CLAUDE_CONFIG_DIR/.olympus-config.json"
...
if [ -f "$SETTINGS_FILE" ] && command -v jq &> /dev/null; then
    ...
    # Remove Olympus-specific hooks from settings.json
    # This removes hooks that reference olympus hook scripts
    TEMP_SETTINGS=$(mktemp)

    # Use jq to filter out Olympus hooks
    jq '
      # Remove Olympus hooks from UserPromptSubmit
      ...
      # Remove Olympus hooks from Stop
      if .hooks.Stop then
        .hooks.Stop |= map(
          if .hooks then
            .hooks |= map(select(.command | (contains("keyword-detector.sh") or contains("silent-auto-update.sh") or contains("stop-continuation.sh")) | not))
    ...
    if [ $? -eq 0 ] && [ -s "$TEMP_SETTINGS" ]; then
        ...
        echo -e "${GREEN}✓ Removed Olympus hooks from settings.json${NC}"
        echo -e "${YELLOW}  Backup saved to: $SETTINGS_FILE.bak${NC}"
    else
        rm -f "$TEMP_SETTINGS"
        echo -e "${YELLOW}⚠ Could not modify settings.json automatically${NC}"
        echo "  Please manually remove Olympus hooks from the 'hooks' section"
    fi
else
...
# Remove .olympus directory if it exists (plans, notepads, drafts)
if [ -d "$CLAUDE_CONFIG_DIR/../.olympus" ] || [ -d ".olympus" ]; then
    echo -e "${YELLOW}Note: .olympus directory (plans/notepads) was not removed.${NC}"
