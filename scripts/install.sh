...
# Clean up any existing directory-based commands (convert to flat file format)
# This handles migration from older directory-based installations
COMMANDS_TO_MIGRATE="ultrawork deepsearch analyze olympus ascent prometheus review"
for cmd in $COMMANDS_TO_MIGRATE; do
  if [ -d "$CLAUDE_CONFIG_DIR/commands/$cmd" ]; then
    echo -e "${YELLOW}  Migrating $cmd/ directory to $cmd.md${NC}"
    rm -rf "$CLAUDE_CONFIG_DIR/commands/$cmd"
  fi
done
...
# Ascent Command (persistence loop with conductor mode)
cat > "$CLAUDE_CONFIG_DIR/commands/ascent.md" << 'CMD_EOF'
---
...
You have entered The Ascent - an INESCAPABLE development cycle that binds you to your task until VERIFIED completion. There is no early exit. There is no giving up. The only way out is through.
...
The Ascent has been cancelled. You can stop working on the current task.
...
(
  ...
  REPO_URL="https://raw.githubusercontent.com/mikev10/olympus/main"
...
| **Guarantee** | ascent | ENSURE completion |
...
| + must complete | `olympus + ascent` | User emphasizes completion |
...
- Any skill + completion emphasis → Add **ascent**
...
| `ascent` | Cannot stop until verified complete |
...
→ prometheus (first) → olympus + ascent (after plan)
...
## What You Do vs. Delegate (HARD RULES)

**CRITICAL: These are NOT suggestions. Violations waste tokens and break orchestration.**

| Action | Do Directly | Delegate | Agent |
|--------|-------------|----------|-------|
| Read single file | Yes | - | - |
| Quick search (<10 results) | Yes | - | - |
| Status/verification checks | Yes | - | - |
| Single file, <10 lines | Yes | - | - |
| Multi-file code changes | **NEVER** | **MUST** | \`olympian\` |
| UI/component work | **NEVER** | **MUST** | \`frontend-engineer\` |
| Complex analysis/debugging | **NEVER** | **MUST** | \`oracle\` |
| Deep codebase exploration | **NEVER** | **MUST** | \`explore\` |

**Self-Check**: If you're about to Read→Edit more than one file, STOP. Delegate instead.
...
echo "  curl -fsSL https://raw.githubusercontent.com/mikev10/olympus/main/scripts/install.sh | bash"
echo ""
