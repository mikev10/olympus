    ...
    if (ascentState?.active) {
      ...
      if (verificationState?.pending) {
        ...
        console.log(JSON.stringify({
          continue: false,
          stopReason: `<ascent-verification>
...
`
      ...
      if (iteration < maxIter) {
        ...
        console.log(JSON.stringify({
          continue: false,
          stopReason: `<ascent-continuation>
...
`
    ...
    if (ultraworkState?.active && incompleteCount > 0) {
      ...
      if (newCount > maxReinforcements) {
        console.log(JSON.stringify({
          continue: true,
          stopReason: `[ULTRAWORK ESCAPE] Maximum reinforcements (${maxReinforcements}) reached. Allowing stop despite ${incompleteCount} incomplete todos. If tasks are genuinely stuck, consider cancelling them or asking the user for help.`
        }));
        return;
      }
      ...
      console.log(JSON.stringify({
        continue: false,
        stopReason: `<ultrawork-persistence>
...
`
    ...
    if (incompleteCount > 0) {
      ...
      const contFile = join(directory, '.olympus', 'continuation-count.json');
      ...
      if (contState.count > maxContinuations) {
        console.log(JSON.stringify({
          continue: true,
          stopReason: `[TODO ESCAPE] Maximum continuation attempts (${maxContinuations}) reached. Allowing stop despite ${incompleteCount} incomplete todos. Tasks may be stuck - consider reviewing and clearing them.`
        }));
        return;
      }
