...
const testCases = [
  ...
  { prompt: 'Update the user list component to show email addresses', agent: 'olympian', expectedTier: 'MEDIUM' },

  // HIGH tier - risky refactoring (detected via keywords)
  { prompt: 'Refactor the user service to use the new database schema and add migrations', agent: 'olympian', expectedTier: 'HIGH' },
  ...
  { prompt: 'Implement the user profile page', agent: 'olympian', expectedTier: 'LOW' },
...
const agents = ['oracle', 'prometheus', 'momus', 'explore', 'document-writer', 'frontend-engineer', 'olympian'];
...
const modelTestCases = [
  ...
  { agent: 'olympian', prompt: 'Fix this typo in the README', expectedModel: 'haiku' },
  { agent: 'olympian', prompt: 'Refactor payment system with migration scripts for production data', expectedModel: 'opus' },
  ...
  { agent: 'orchestrator-olympus', prompt: 'Simple task', expectedModel: 'opus' },
];
...
const allAgents = ['orchestrator-olympus', 'oracle', 'prometheus', 'momus', 'metis', 'explore', 'olympian', 'frontend-engineer'];
...
const analysis = analyzeTaskComplexity(analysisPrompt, 'olympian');
